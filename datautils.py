...
def get_c4_val(nsamples,  seed, seqlen, model , data_path, cached=True):
    print('loading data')
    # data_path = os.path.expanduser('~') + '/qiht/models/'
    if not cached:
    ...
    from transformers import AutoTokenizer
    print(model)
    tokenizer = AutoTokenizer.from_pretrained(model, use_fast=False, cache_dir=data_path)
...
def get_loaders(
    name, data_path, nsamples=128, seed=0, seqlen=2048, model=''
):
    ...
    if 'c4' in name:
        return get_c4_val(nsamples, seed, seqlen, model, data_path)
