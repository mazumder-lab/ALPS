...
@torch.no_grad()
def opt_sequential(model, dataloader, dev, nsamples=128):
    print('Starting ...')
...
if __name__ == '__main__':
    ...
    tick = time.time()
    sp = opt_sequential(model, dataloader, DEV, nsamples=args.nsamples)
    ...
    for dataset in datasets: 
        ...
        ppl = opt_eval(model, testloader, DEV) # Evaluate on test data
        results[dataset] = ppl
